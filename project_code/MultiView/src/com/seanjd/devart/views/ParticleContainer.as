package com.seanjd.devart.views {
	...
	public class ParticleContainer extends ObjectContainer3D {
		...
		static public const PARTICLE_COUNT:uint = 4000;
		...
		public function ParticleContainer(scene:MainScene) {
			super();
			
			material = new ColorMaterial(0x52faa2);
			material.lightPicker = scene.lightPicker;
		...
		private function initParticleParam(prop:ParticleProperties):void{
			...
			var y:Number =  Math.sin(percent * Math.PI * 2 * 5);

			x = Math.sin(Math.random() * Math.PI * 2) * prop.index / 5;
			y = Math.sin(Math.random() * Math.PI * 2) * prop.index / 15;
